...
export function MatrixCell({ cell, index, atomCount, firstAtomId, atomNames = [] }: Props) {
  const c = STAGE_HEX[cell.column as StageColumnId] || STAGE_HEX.discover
    ...
    <>
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] font-mono font-semibold tracking-wider text-neutral-400 dark:text-neutral-500">
            STEP
          </span>
        ...
        {atomCount > 0 && (
          <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-white/80 dark:bg-neutral-800/80 border border-neutral-200/50 dark:border-neutral-700/50">
            ...
            <span className="text-[10px] font-semibold tabular-nums" style={{ color: c.hex }}>
              {atomCount}
            </span>
          </div>
        )}
      ...
      <div>
        ...
        <div className="text-[11px] text-neutral-500 dark:text-neutral-400 mt-0.5 leading-tight truncate">
          {cell.nameEn}
        </div>
        {cell.tagline ? (
          <div className="text-[10px] text-neutral-400 dark:text-neutral-500 mt-1.5 truncate font-mono">
            {cell.tagline}
          </div>
        ) : (
          <div className="text-[10px] mt-1.5">&nbsp;</div>
        )}
      </div>
      ...
      {!hasAtoms && (
        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <div className="flex items-center gap-0.5 px-1.5 h-5 rounded-md bg-white dark:bg-neutral-800 shadow-lg border border-neutral-200 dark:border-neutral-700 text-[10px] text-neutral-500">
          ...
          {atomNames.length > 0 ? (
            <ul className="space-y-0.5">
              {atomNames.slice(0, 5).map((n) => (
                <li
                  key={n}
                  className="text-[11px] text-neutral-700 dark:text-neutral-300 truncate"
                >
                  · {n}
                </li>
              ))}
            ...
            <div className="text-[11px] text-neutral-400">暂无原子</div>
          )}
        </div>
      </div>
    </>
  ...
  return (
    ...
    >
      {hasAtoms && firstAtomId ? (
        <Link to={`/atoms/${firstAtomId}`} className={className} style={style}>
          {inner}
        </Link>
      ) : (
