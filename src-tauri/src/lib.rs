...
fn init_check_skill_installation(app: AppHandle) -> Result<InitStepResult, String> {
    ...
    let cli_ok = cli_shim_unix.exists() || cli_shim_win.exists();
...
/// Tiny std-only ISO-8601 timestamp using SystemTime. We don't pull `chrono`.
fn chrono_now_iso() -> String {
    ...
    let secs = SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    // Very basic UTC formatter — good enough for an audit field.
    format!("@{}", secs)
}
