...
- [ ] B1. 新增 `scripts/lib/evolution.mjs` 模块,封装 `computeStaleness` / `updateAccessTime` / `detectCollision` / `applySupersede` / `applyArchive` / `detectPruneCandidates` / **`applyProfileEvolution`** (D-008, profile 单例覆写 + previous_versions 入栈) 7 个函数。`applyProfileEvolution(newSnapshot, trigger)` 签名在 design §4.2.1 定义, 由 bootstrap-skill change 在其实施 PR 中调用
