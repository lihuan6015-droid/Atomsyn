...
- 2026-04-27 · 2026-04-bootstrap-tools · `~/.atomsyn/bootstrap-sessions/<id>.json` (no schema file, ad-hoc JSON) · additive · 新增 `options.mode: "agentic" | "funnel"` (default `agentic` per D-001) · 新增 `agent_trace: BootstrapAgentTraceEntry[]` (D-003: agentic 模式下每次 tool 调用追加一条 `{ts, tool, args, result_summary, duration_ms, error?}`; v1 / funnel 模式留空) · 新增 `phase3_progress.mode` 反映实际跑的模式 · 新增 `options.mode_fallback_from` (agentic→funnel 自动 fallback 时记录) · v1 session 加载时缺失 mode 默认 funnel, 缺失 agent_trace 默认 [], GUI / Tauri / CLI 全兼容
