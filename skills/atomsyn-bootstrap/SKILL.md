...
description: "把用户硬盘上散落的过程文档 (markdown / docx / pdf / txt / 历史聊天导出 / 源代码注释) 引导式地导入 Atomsyn 知识库, 产出 1 条 profile atom + N 条 experience/fragment atom。bootstrap-tools v2 默认 agentic 模式: LLM 用 ls/stat/glob/grep/read 5 个工具自主探索目录树; funnel 模式 (`--mode funnel`) 是 v1 硬编码 5 层串行, 作为兜底。隐私优先: 14 条敏感关键字 + .atomsynignore。两步协议: dry-run 出 markdown 让用户校对, commit 才真正写入。用户说 '初始化 atomsyn / bootstrap atomsyn / 把 ~/X 倒进来 / 从我之前的笔记导入 / 第一次用 atomsyn' 或在 GUI 聊天用 `/bootstrap` 时触发。"
...
- **B-I8 · prompt 模板锁定** (D-012): bootstrap 内部使用的 LLM prompt 模板从 `scripts/bootstrap/prompts/*.md` 加载 (8 份: triage/sampling/deep-dive-l1-l2/deep-dive-l3/deep-dive-l4/deep-dive-l5/commit + bootstrap-tools v2 新增 `agentic-deepdive.md`), **不可被 ENV 或配置文件 override** (v1)。如果用户问"能改 prompt 吗", 告诉他"v1 不开放 override, 高级用户暂时只能 fork 仓库; v2 视反馈再考虑"。
- **B-I9 · agentic 沙箱** (bootstrap-tools D-006): agentic 模式下 LLM 通过 `ls/stat/glob/grep/read` 工具集探索时, 所有路径必须 startsWith 用户在 `--path` 中明确给的根 (sandboxRoots); 越界抛 `SANDBOX_VIOLATION`. `read` 走 extractor 链 + privacy 链, 不绕过敏感扫描.
- **B-I10 · agentic loop 双重上限** (bootstrap-tools D-009): tool-use loop 同时受 `maxLoops=30` 和 `maxTokens=100_000` 约束, 任一触发即终止; 失败时自动 fallback funnel 一次 + WARN (D-008).
...
- **GUI 入口** (bootstrap-tools v2): 用户在聊天输入框打 `/bootstrap` (打开向导) 或 `/bootstrap <path>` (向导预填路径); 粘贴绝对路径触发 PathDetectionBanner 接受时同样进入向导
...
| **DEEP DIVE agentic (default, bootstrap-tools D-001)** | 5-30 (loop rounds) | ≤ 100k 累计 input+output (硬上限 D-009) | ≤ 100k (~$0.30 Sonnet) |
| DEEP DIVE funnel 串行 (`--mode funnel`) | N (= 文件数) | ≤ 8k input + 2k output | ~10M (1000 × 10k) |
| DEEP DIVE funnel `--parallel` | N (4 路 sub-agent) | 同上 | **~40M (4x cost)** |
...
```bash
# 默认 agentic 模式 (bootstrap-tools D-001), LLM 用 ls/glob/grep/read 自主探索
atomsyn-cli bootstrap --resume <session-id> --phase deep-dive --dry-run [--user-correction "..."]

# 兜底: 强制走 v1 funnel (硬编码 5 层 per-file pass, D-008 保留)
atomsyn-cli bootstrap --resume <session-id> --phase deep-dive --dry-run --mode funnel
```
...
**v2 agentic 输出额外含**: 末尾"Agent 探索轨迹"折叠章节 (LLM 调用 ls/glob/read 的时间线), 让用户审计探索质量。session JSON 的 `agent_trace[]` 字段同步存有结构化 trace, GUI BootstrapWizard DryrunScreen 也会渲染。
