...
function profileSingletonFile(dataDir: string): string {
        ...
        try {
          ...
          if (parts[0] === 'frameworks') {
            ...
            if (method === 'GET' && parts.length === 3 && parts[2] === 'stats') {
              ...
              if (!existsSync(fwFile)) return send(res, 404, { error: 'not found' })
              const fw = JSON.parse(await fs.readFile(fwFile, 'utf-8'))

              // Extract all nodes from any layout type
          ...
          if (parts[0] === 'atoms') {
            ...
            if (method === 'POST' && parts.length === 3 && parts[1] === 'profile' && parts[2] === 'restore') {
              ...
              if (!target) return send(res, 400, { error: `version ${version} not in previous_versions` })
            ...
            if (method === 'PATCH' && parts.length === 3 && parts[2] === 'calibrate') {
              ...
              const atom = JSON.parse(await fs.readFile(file, 'utf-8'))
              atom.stats = atom.stats || {}
              if (calBody.locked !== undefined) atom.stats.locked = calBody.locked
              if (calBody.confidence !== undefined) atom.confidence = calBody.confidence
              // If locking, set confidence to 1.0
              if (calBody.locked === true) atom.confidence = 1.0
            ...
            if (method === 'POST' && parts.length === 3 && parts[2] === 'calibrate-profile') {
              ...
              const calBody = await readBody(req)
              const profile = await readProfileSingleton(dataDir)
              if (!profile) return send(res, 404, { error: 'profile not found — run bootstrap first' })
              if (id !== profile.id) {
                return send(res, 400, { error: `id ${id} is not the active profile (${profile.id})` })
              }
              const newSnapshot: Record<string, unknown> = {}
              if (calBody?.identity !== undefined) newSnapshot.identity = calBody.identity
              if (calBody?.preferences !== undefined) newSnapshot.preferences = calBody.preferences
              if (calBody?.knowledge_domains !== undefined) newSnapshot.knowledge_domains = calBody.knowledge_domains
              if (calBody?.recurring_patterns !== undefined) newSnapshot.recurring_patterns = calBody.recurring_patterns
            ...
            if (method === 'POST' && parts.length === 3 && parts[2] === 'supersede') {
              ...
              if (!oldFile) return send(res, 404, { error: 'old atom not found' })
              const oldAtom = JSON.parse(await fs.readFile(oldFile, 'utf-8'))
              if (oldAtom.stats?.locked) return send(res, 423, { error: 'old atom is locked' })
              if (oldAtom.archivedAt) return send(res, 409, { error: 'old atom is already archived' })
            ...
            if (method === 'POST' && parts.length === 3 && parts[2] === 'archive') {
              ...
              if (!file) return send(res, 404, { error: 'atom not found' })
              const atom = JSON.parse(await fs.readFile(file, 'utf-8'))
              if (atom.stats?.locked) return send(res, 423, { error: 'atom is locked' })
              const now = new Date().toISOString()
              atom.archivedAt = now
            ...
            if (method === 'POST' && parts.length === 3 && parts[2] === 'restore') {
              ...
              if (!file) return send(res, 404, { error: 'atom not found' })
              const atom = JSON.parse(await fs.readFile(file, 'utf-8'))
              if (!atom.archivedAt) return send(res, 400, { error: 'atom is not archived' })
          ...
          if (parts[0] === 'notes') {
            ...
            async function loadNoteContent(noteDir: string): Promise<string> {
              ...
              try { return await fs.readFile(contentFile, 'utf-8') } catch { return '' }
            }

            async function loadFullNote(noteDir: string, basePath: string): Promise<any> {
            ...
            async function collectAllNotes(dir: string, basePath: string): Promise<any[]> {
              const notes: any[] = []
            ...
            function groupDirPath(groupId: string): string {
            ...
            if (parts.length === 3 && parts[1] === 'trash' && method === 'DELETE') {
              ...
              if (!noteDir) return send(res, 404, { error: 'note not found in trash' })
            ...
            if (parts.length >= 2 && parts[1] !== 'meta' && parts[1] !== 'trash') {
              ...
              if (parts.length === 3 && parts[2] === 'crystallize-cache' && method === 'GET') {
                ...
                if (!cache) return send(res, 200, null)
              ...
              if (parts.length === 3 && parts[2] === 'crystallize-cache' && method === 'DELETE') {
                ...
                if (!noteDir) return send(res, 404, { error: 'note not found' })
                const cacheFile = path.join(noteDir, 'crystallize-cache.json')
                try { await fs.rm(cacheFile) } catch { /* file may not exist */ }
                return send(res, 200, { ok: true })
              }
              ...
              if (parts.length === 3 && parts[2] === 'attachment' && method === 'POST') {
                ...
                if (!filename || !base64) return send(res, 400, { error: 'filename and base64 required' })
          ...
          if (parts[0] === 'analysis') {
            ...
            if (parts[1] === 'reports') {
              ...
              if (!parts[2] && method === 'GET') {
                ...
                reports.sort((a: any, b: any) => (b.createdAt || '').localeCompare(a.createdAt || ''))
              ...
              if (parts[2]) {
                ...
                if (method === 'GET') {
                  if (!existsSync(reportFile)) return send(res, 404, { error: 'report not found' })
                ...
                if (method === 'DELETE') {
                  try { await fs.rm(reportFile) } catch { /* may not exist */ }
                  return send(res, 200, { ok: true })
                }
              }
            }
          ...
          if (parts[0] === 'chat') {
            ...
            if (parts[1] === 'soul') {
              ...
              if (method === 'GET') {
                let content = ''
                try { content = await fs.readFile(soulFile, 'utf8') } catch { /* empty */ }
            ...
            if (parts[1] === 'agents') {
              ...
              if (method === 'GET') {
                let content = ''
                try { content = await fs.readFile(agentsFile, 'utf8') } catch { /* empty */ }
                return send(res, 200, { content })
              }
            }
            ...
            if (parts[1] === 'sessions') {
              ...
              async function writeSessionIndex(idx: { sessions: any[] }): Promise<void> {
              ...
              if (parts[2]) {
                ...
                if (method === 'GET') {
                  if (!existsSync(sessFile)) return send(res, 404, { error: 'session not found' })
                ...
                if (method === 'PUT') {
                  ...
                  try { session = JSON.parse(await fs.readFile(sessFile, 'utf8')) } catch { /* new */ }
                ...
                if (method === 'DELETE') {
                  try { await fs.rm(sessFile) } catch { /* may not exist */ }
                  const idx = await readSessionIndex()
